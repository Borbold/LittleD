...
*/
db_int parseexpr_pop(db_int type, db_eetnode_t **exprp, db_eetnode_t *stack,
                     db_eetnode_t **stack_topp, db_int *sizep, db_uint8 limit,
                     db_query_mm_t *mmp) {
  while (*stack_topp != stack && limit != (*stack_topp)->type &&
         (type == -1 || compareopprecedence(type, (*stack_topp)->type) >= 0)) {
    if (isunary(type) ||
        (isunary((*stack_topp)->type) && type != -1 &&
         compareopprecedence(type, (*stack_topp)->type) >= 0)) {
      return 1;
...
#ifndef ISFUNC
#define ISFUNC(t)                                                              \
  ((db_uint8)DB_EETNODE_FUNC_LENGTH_DBSTRING >= (t) &&                         \
   (db_uint8)DB_EETNODE_FUNC_LENGTH_DBSTRING <= (t))
#else
...
db_int parseexpression(db_eetnode_t **exprp, db_lexer_t *lexerp, db_int start,
  ...
  while (lexerp->offset < end && 1 == lexer_next(lexerp) &&
         lexerp->token.start < end) {
    if ((db_uint8)DB_LEXER_TT_INT == lexerp->token.type) {
      ...
      db_eetnode_dbint_t *newnodep =
          POINTERATNBYTES(*exprp, size, db_eetnode_dbint_t *);
      ...
      db_eetnode_dbstring_t *newnodep =
          POINTERATNBYTES(*exprp, size, db_eetnode_dbstring_t *);
      ...
      db_eetnode_placeholder_t *newnodep =
          POINTERATNBYTES(*exprp, size, db_eetnode_placeholder_t *);
      ...
      db_eetnode_attr_t *newnodep =
          POINTERATNBYTES(*exprp, size, db_eetnode_attr_t *);
      ...
      */
      if ((db_uint8)DB_EETNODE_OP_SUB == type &&
          (DB_EETNODE_COUNT == lasttype || DB_EETNODE_LPAREN == lasttype ||
           ...
           (DB_EETNODE_OP_UNARYNEG <= lasttype &&
            DB_EETNODE_OP_ISNULL >= lasttype))) {
      ...
      switch (parseexpr_pop(type, exprp, stack, &stack_top, &size,
                            (db_uint8)DB_EETNODE_COUNT, mmp)) {
      case 1:
        break;
      default:
        return -1;
      ...
      switch (parseexpr_pop(-1, exprp, stack, &stack_top, &size,
                            (db_uint8)DB_EETNODE_LPAREN, mmp)) {
      case 1:
        break;
      default:
        return -1;
      ...
      if (lexerp->offset < end && 1 == lexer_next(lexerp) &&
          lexerp->token.start < end &&
          (db_uint8)DB_EETNODE_LPAREN == lexerp->token.bcode) {
      ...
      if (stack_top == stack) {
        ...
        if (NULL != (stack_top = db_qmm_bextend(
                         mmp, (db_int)(-sizeof(db_eetnode_t)))) &&
            stack_top != stack) {
          if (ISFUNC(stack_top->type)) {
            if (1 != db_qmm_fextend(mmp, sizeof(db_eetnode_t))) {
              return -1;
            }
            db_eetnode_t *newnodep =
                POINTERATNBYTES(*exprp, size, db_eetnode_t *);
            ...
            *newnodep = *stack_top;
            ...
            db_eetnode_aggr_temp_t *newnodep =
                POINTERATNBYTES(*exprp, size, db_eetnode_aggr_temp_t *);
            size += sizeof(db_eetnode_aggr_temp_t);
            *newnodep = *((db_eetnode_aggr_temp_t *)stack_top);
            stack_top =
                db_qmm_bextend(mmp, (db_int)(-sizeof(db_eetnode_aggr_temp_t)));
          }
...
TODO: We have implicitly assumed that no projection involving complex
expressions will occur below the joins.  this may not be a good assumption, I
don't know. -I mean things like "SELECT 2*T.x".  "2*T.x" is complex.
*/
/**
...
@param		depth		The depth of the evaluation point in the
                                execution tree.
*/
db_int setupattribute(db_eetnode_attr_t *attrnodep, db_lexer_t *lexerp,
                      db_op_base_t *evalpoint, scan_t *scans, db_uint8 numscans,
                      int depth) {
  ...
  if (DB_NTJOIN == evalpoint->type || DB_OSIJOIN == evalpoint->type) {
    ...
    /* Try left child. */
    if (1 == (retval = setupattribute(attrnodep, lexerp,
                                      ((ntjoin_t *)evalpoint)->lchild, scans,
                                      numscans, depth + 1))) {
    } else if (-1 == retval) {
      return retval;
    }
    /* Try right child. */
    else if (1 == (retval = setupattribute(attrnodep, lexerp,
                                           ((ntjoin_t *)evalpoint)->rchild,
                                           scans, numscans, depth + 1))) {
      if (depth > 0) {
  ...
  } else if ((DB_PROJECT == evalpoint->type ||
              DB_AGGREGATE == evalpoint->type) &&
             depth > 0) {
    // TODO What to do here?  Simply error?  Not clear.
  } else if (1 == numopchildren(evalpoint)) {
    db_int retval = setupattribute(attrnodep, lexerp,
                                   ((db_op_onechild_t *)evalpoint)->child,
                                   scans, numscans, depth + 1);
    if (1 == retval) {
      attrnodep->tuple_pos = 0;
    }
    ...
    // TODO: Pull this out so ambiguity/existence check done only once per
    // attribute reference.
    if (1 == attrnodep->pos) {
      ...
      if (0 == count) {
        DB_ERROR_MESSAGE("attribute does not exist", attrnodep->tokenstart,
                         lexerp->command);
        return -1;
      } else if (count > 1) {
        DB_ERROR_MESSAGE("column name ambiguous", attrnodep->tokenstart,
                         lexerp->command);
        return -1;
      }
    ...
    /* Check if this is the correct table. */
      ...
      if (!token_stringequal(&temptoken, tablename, sizeof(tablename) - 1,
                             lexerp, 0)) {
        return 0;
      }
      ...
      DB_ERROR_MESSAGE("unsupported - database identifier",
                       attrnodep->tokenstart, lexerp->command);
      return -1;
    }
...
db_int verifysetupattributes(db_eet_t *eetp, db_lexer_t *lexerp,
                             db_op_base_t *evalpoint, scan_t *scans,
                             db_uint8 numscans, int startdepth) {
  db_eetnode_t *cursor = eetp->nodes;
  db_int retval;
  while (POINTERBYTEDIST(cursor, eetp->nodes) < eetp->size) {
    if (DB_EETNODE_ATTR == cursor->type) {
      retval = setupattribute((db_eetnode_attr_t *)cursor, lexerp, evalpoint,
                              scans, numscans, startdepth);
      if (1 != retval) {
        return retval;
      }
    }
