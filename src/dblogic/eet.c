...
db_int createnewheader(relation_header_t *new_hp, relation_header_t *old_hp,
                       db_eet_t *exprs, db_uint8 num_exprs,
                       db_query_mm_t *mmp) {
  ...
  for (k = 0; k < (db_int)num_exprs; ++k, ++i) {
    ...
    if (NULL == np) {
      ...
      db_int j;
      /* For each expression ... */
      for (j = exprs[k].stack_size; j < exprs[k].stack_size + exprs[k].size;
           ++j, ++i) {
      ...
      --i;
    } else if (np->type == DB_EETNODE_ATTR &&
               sizeof(db_eetnode_attr_t) == exprs[k].size) {
      /* Translate information from old header to new header.
       */
      new_hp->size_name[i] =
          old_hp->size_name[(db_int)(((db_eetnode_attr_t *)np)->pos)];
      new_hp->names[i] =
          old_hp->names[(db_int)(((db_eetnode_attr_t *)np)->pos)];
      new_hp->types[i] =
          old_hp->types[(db_int)(((db_eetnode_attr_t *)np)->pos)];
      /* Current tuple size is next offset */
      new_hp->offsets[i] = new_hp->tuple_size;
      new_hp->sizes[i] =
          old_hp->sizes[(db_int)(((db_eetnode_attr_t *)np)->pos)];
      new_hp->tuple_size += new_hp->sizes[i];
    } else {
      ...
      if (DB_EETNODE_CONST_NULL == type) {
        ...
        */
        ...
        /* Current tuple size is next offset */
        new_hp->offsets[i] = new_hp->tuple_size;
        new_hp->sizes[i] = strlen(((db_eetnode_dbstring_t *)np)->string) + 1;
        new_hp->tuple_size += new_hp->sizes[i];
      } else {
        // TODO: Free stuff.
        return -1;
...
db_int advanceeetnodepointer(db_eetnode_t **npp, db_int num_times) {
  ...
  for (i = 0; i < num_times; ++i) {
    ...
    if ((*npp)->type == DB_EETNODE_ATTR)
      MOVEPOINTERNUNITS(*npp, *npp, 1, db_eetnode_t *, db_eetnode_attr_t *);
...
db_uint8 funcopreturntype(db_uint8 *array) {
  if (*array >= (db_uint8)DB_EETNODE_OP_NOT &&
      *array <= (db_uint8)DB_EETNODE_OP_BCOMP) {
    array++;
  ...
  } else if (*array >= (db_uint8)DB_EETNODE_OP_BAND &&
             *array <= (db_uint8)DB_EETNODE_OP_MOD) {
    array++;
    if ((db_uint8)DB_EETNODE_CONST_DBINT == *array) {
      array++;
      if ((db_uint8)DB_EETNODE_CONST_DBINT == *array) {
  ...
  } else if (*array >= (db_uint8)DB_EETNODE_OP_ADD &&
             *array <= (db_uint8)DB_EETNODE_OP_DIV) {
    array++;
    if ((db_uint8)DB_EETNODE_CONST_DBINT == *array) {
      array++;
      if ((db_uint8)DB_EETNODE_CONST_DBINT == *array) {
  ...
  } else if (*array >= (db_uint8)DB_EETNODE_OP_LT &&
             *array <= (db_uint8)DB_EETNODE_OP_EQ) {
    array++;
    if ((db_uint8)DB_EETNODE_CONST_DBINT == *array) {
      array++;
      if ((db_uint8)DB_EETNODE_CONST_DBINT == *array) {
...
db_int subexpr_type_addr(db_eetnode_t *np, db_uint8 *type,
  ...
  /* Initially, set the value of type to a value greater than any other type's
   * values. */
  ...
  do {
    ...
    case (db_uint8)DB_EETNODE_ALIAS:
      ...
      /* This is a little ugly, but it should work.
         ...
         subexpression. */
      subexpr_type_addr(
          ((db_eetnode_alias_t *)np)->expr->nodes, stack_top, NULL,
          (((db_eetnode_alias_t *)np)->expr->stack_size) / sizeof(db_eetnode_t),
          hpa, mmp);
    ...
    while (stack_top > stack) {
      ...
      if (*stack_lookup >= (db_uint8)DB_EETNODE_OP_NOT &&
          *stack_lookup < (db_uint8)DB_EETNODE_COUNT) {
        if (*stack_lookup >= (db_uint8)DB_EETNODE_OP_UNARYNEG &&
            *stack_lookup <= (db_uint8)DB_EETNODE_OP_BCOMP) {
          if (count < 1) {
            break;
          }
        } else if (*stack_lookup >= (db_uint8)DB_EETNODE_OP_BAND &&
                   *stack_lookup <= (db_uint8)DB_EETNODE_OP_MOD) {
          if (count < 2) {
            break;
          }
        } else if (*stack_lookup >= (db_uint8)DB_EETNODE_OP_ADD &&
                   *stack_lookup <= (db_uint8)DB_EETNODE_OP_DIV) {
          if (count < 2) {
            break;
          }
        } else if (*stack_lookup >= (db_uint8)DB_EETNODE_OP_LT &&
                   *stack_lookup <= (db_uint8)DB_EETNODE_OP_EQ) {
          if (count < 2) {
            break;
          }
        } else if (*stack_lookup >= (db_uint8)DB_EETNODE_OP_ISNULL &&
                   *stack_lookup <= (db_uint8)DB_EETNODE_OP_ISNULL) {
          if (count < 1) {
            break;
          }
        } else if (*stack_lookup >= (db_uint8)DB_EETNODE_FUNC_LENGTH_DBSTRING &&
                   *stack_lookup <= (db_uint8)DB_EETNODE_FUNC_LENGTH_DBSTRING) {
          if (count < 1) {
            break;
          }
        }
...
*/
// TODO: Implement future types for function.  Might need to be careful w.r.t.
// scalable sized integers.
...
db_eetnode_t *eet_promote(db_eetnode_t *values, db_uint8 numvalues,
                          db_uint8 *promotedto_p, db_query_mm_t *mmp) {
  ...
  *promotedto_p = 0;
  ...
  // TODO: At some point, we will have to deal with allocating space for
  // strings.
  db_eetnode_t *newvals = db_qmm_balloc(mmp, numvalues * typesize);
  if (NULL == newvals)
    return NULL;
  db_eetnode_t *newval;
  MOVEPOINTERNBYTES(newval, newvals, typesize * (numvalues - 1),
                    db_eetnode_t *);
  value = values;
  processed = 0;
...
db_int eet_numrequiredvals(db_uint8 type) {
  if ((db_uint8)DB_EETNODE_OP_BAND <= type &&
      (db_uint8)DB_EETNODE_OP_EQ >= type) {
    return 2;
  } else {
    return 1;
  }
...
db_int eet_evaloporfunc(db_eetnode_t **valuesp, db_uint8 type,
                        db_uint8 numvalues, db_uint8 promotedto,
                        db_query_mm_t *mmp) {
  size_t typesize;
  switch (promotedto) {
  case (db_uint8)DB_EETNODE_CONST_DBINT:
  ...
  if ((db_uint8)DB_EETNODE_CONST_NULL == promotedto) {
    if ((db_uint8)DB_EETNODE_OP_ISNULL == type) {
      *valuesp = db_qmm_bextend(mmp, sizeof(db_eetnode_dbint_t) -
                                         sizeof(db_eetnode_t));
  ...
  // TODO: Move AND, OR to be away from others, to make this nicer.
           ...
           ((db_uint8)DB_EETNODE_CONST_NULL == (*valuesp)->type ||
            (eet_numrequiredvals(type) > 1 &&
             (db_uint8)DB_EETNODE_CONST_NULL ==
                 POINTERATNBYTES(*valuesp, typesize, db_eetnode_t *)->type))) {
    (*valuesp)->type = DB_EETNODE_CONST_NULL;
...
db_int evaluate_eet(db_eet_t *exprp, void *rp, db_tuple_t **tp,
                    relation_header_t **hp, db_uint8 start_from,
                    db_query_mm_t *mmp) {
  ...
  while (POINTERBYTEDIST(cursor, exprp->nodes) < exprp->size) {
    if ((db_uint8)DB_EETNODE_CONST_NULL == cursor->type) {
      ...
      *((db_eetnode_dbstring_t *)stack_top) =
      ...
      db_uint8 attr_type = gettypebypos(
          hp[(db_int)(((db_eetnode_attr_t *)cursor)->tuple_pos)], pos);
      db_uint8 isnull = 0;
      if (NULL != rp) {
      ...
      /* If NULL == rp, we are only finding assumed type of expression, adjust
       * accordingly. */
      if (1 == isnull) {
        stack_top = db_qmm_bextend(mmp, sizeof(db_eetnode_t));
    ...
    } else /* It is an operator. */
    {
      ...
      db_eetnode_t *newvalues =
          eet_promote(stack_top, numreqvals, &promotedto, mmp);
      if (NULL == newvalues) {
        db_qmm_bfree(mmp, stack_top);
        return -1;
      }

      switch (eet_evaloporfunc(&newvalues, cursor->type, numreqvals, promotedto,
                               mmp)) {
      case 1:
        break;
      default:
