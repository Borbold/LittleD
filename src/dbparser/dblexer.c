...
static struct keyword clauses[] = {
    {"CREATE", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_CLAUSE_CREATE},
    {"INSERT", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_CLAUSE_INSERT},
    {"DROP", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_UNIMPLEMENTED},
    {"DELETE", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_CLAUSE_DELETE},
    {"UPDATE", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_CLAUSE_UPDATE},
    {"INSERT", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_UNIMPLEMENTED},
    ...
    {"GROUP", DB_LEXER_TOKENINFO_UNIMPORTANT,
     DB_LEXER_TOKENBCODE_CLAUSE_GROUPBY},
    {"HAVING", DB_LEXER_TOKENINFO_UNIMPORTANT,
     DB_LEXER_TOKENBCODE_CLAUSE_HAVING},
    {"ORDER", DB_LEXER_TOKENINFO_UNIMPORTANT,
     DB_LEXER_TOKENBCODE_CLAUSE_ORDERBY},
    {"SELECT", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_CLAUSE_SELECT},
    {"EXPLAIN", DB_LEXER_TOKENINFO_COMMANDCLAUSE,
     DB_LEXER_TOKENBCODE_UNIMPLEMENTED}};
...
static struct keyword reservedwords[] = {
    ...
    {"AS", DB_LEXER_TOKENINFO_ALIAS_INDICATOR,
     DB_LEXER_TOKENBCODE_UNIMPLEMENTED},
    ...
    {"STRING", DB_LEXER_TOKENINFO_TYPE_DBSTRING,
     DB_LEXER_TOKENBCODE_UNIMPORTANT},
    {"DECIMAL", DB_LEXER_TOKENINFO_TYPE_DBDECIMAL,
     DB_LEXER_TOKENBCODE_UNIMPORTANT},
    {"INTO", DB_LEXER_TOKENINFO_UNIMPORTANT, DB_LEXER_TOKENBCODE_UNIMPLEMENTED},
    {"LEFT", DB_LEXER_TOKENINFO_JOINTYPE_SPECIAL,
     DB_LEXER_TOKENBCODE_JOINDECORATOR_LEFT},
    {"RIGHT", DB_LEXER_TOKENINFO_JOINTYPE_SPECIAL,
     DB_LEXER_TOKENBCODE_JOINDECORATOR_RIGHT},
    {"FULL", DB_LEXER_TOKENINFO_JOINTYPE_SPECIAL,
     DB_LEXER_TOKENBCODE_JOINDECORATOR_FULL},
    {"OUTER", DB_LEXER_TOKENINFO_JOINTYPE_SPECIAL,
     DB_LEXER_TOKENBCODE_JOINDECORATOR_OUTER},
    {"NATURAL", DB_LEXER_TOKENINFO_JOINTYPE_SPECIAL,
     DB_LEXER_TOKENBCODE_JOINDECORATOR_NATURAL},
    {"INNER", DB_LEXER_TOKENINFO_JOINTYPE_NORMAL,
     DB_LEXER_TOKENBCODE_JOINDECORATOR_INNER},
    {"CROSS", DB_LEXER_TOKENINFO_JOINTYPE_NORMAL,
     DB_LEXER_TOKENBCODE_JOINDECORATOR_CROSS},
    {"JOIN", DB_LEXER_TOKENINFO_JOINTYPE_NORMAL,
     DB_LEXER_TOKENBCODE_JOIN_ABSOLUTE},
    {"ASC", DB_LEXER_TOKENINFO_UNIMPORTANT, DB_TUPLE_ORDER_ASC},
    {"DESC", DB_LEXER_TOKENINFO_UNIMPORTANT, DB_TUPLE_ORDER_DESC},
    {"TABLE", DB_LEXER_TOKENINFO_LITERAL_TABLE,
     DB_LEXER_TOKENBCODE_UNIMPORTANT},
    {"VALUES", DB_LEXER_TOKENINFO_LITERAL_VALUES,
     DB_LEXER_TOKENBCODE_UNIMPORTANT},
    {"SET", DB_LEXER_TOKENINFO_LITERAL_SET, DB_LEXER_TOKENBCODE_UNIMPORTANT}};
...
static struct keyword functions[] = {
    ...
    {"CONCAT", DB_LEXER_TOKENINFO_UNIMPLEMENTED,
     DB_LEXER_TOKENBCODE_UNIMPLEMENTED}};
...
@param		lexerp		Pointer to the lexer instance that is to be
checked.
...
*/
void moveoffset(db_lexer_t *lexerp, db_int moveby) { lexerp->offset += moveby; }
...
@returns	@c 1 if the lexer's next character is whitespace, @c 0
otherwise.
*/
...
db_int token_stringequal(db_lexer_token_t *tokenp, char *string,
                         db_int stringlength, db_lexer_t *lexerp,
                         db_uint8 casesensitive) {
  ...
  while (i < tokenp->end && string[j] != '\0') {
    if (!casesensitive &&
        (tocapital(lexerp->command[i]) != tocapital(string[j]))) {
      return 0;
...
*/
db_int token_whichelement(db_lexer_token_t *tokenp, db_lexer_t *lexerp,
                          struct keyword *array, int arraysize) {
...
*/
db_int token_iselement(db_lexer_token_t *tokenp, db_lexer_t *lexerp,
                       struct keyword *array, int arraysize, db_int *which) {
  if (NULL == which) {
...
db_int isclause(db_lexer_token_t *tokenp, db_lexer_t *lexerp, db_int *which) {
  return token_iselement(tokenp, lexerp, clauses,
                         sizeof(clauses) / sizeof(struct keyword), which);
}
...
db_int isreserved(db_lexer_token_t *tokenp, db_lexer_t *lexerp, db_int *which) {
  return token_iselement(tokenp, lexerp, reservedwords,
                         sizeof(reservedwords) / sizeof(struct keyword),
                         which) ||
         isclause(tokenp, lexerp, which);
}
...
db_int isoperator(db_lexer_token_t *tokenp, db_lexer_t *lexerp, db_int *which) {
  return token_iselement(tokenp, lexerp, operators,
                         sizeof(operators) / sizeof(struct keyword), which);
}
...
db_int isstartofoperator(db_lexer_token_t *tokenp, db_lexer_t *lexerp) {
  ...
  for (; i < sizeof(operators) / sizeof(struct keyword); ++i) {
    ...
    for (; j < tokenp->end - tokenp->start && j < sizeof(operators[i]); ++j) {
      if (tocapital(lexerp->command[tokenp->start + j]) ==
          tocapital(operators[i].word[j]))
        return 1;
    }
  }
  return 0;
...
db_int isfunction(db_lexer_token_t *tokenp, db_lexer_t *lexerp, db_int *which) {
  return token_iselement(tokenp, lexerp, functions,
                         sizeof(functions) / sizeof(struct keyword), which);
}
...
@returns	@c 0 if the token is an operator, @c 1 otherwise.
*/
db_int isaggrfunction(db_lexer_token_t *tokenp, db_lexer_t *lexerp,
                      db_int *which) {
  return token_iselement(tokenp, lexerp, aggrfunctions,
                         sizeof(aggrfunctions) / sizeof(struct keyword), which);
}
...
@param		which		The index of the token in its appropriate
                                keyword array.
@param		start		The starting offset of the token. Pass a
negative value to not set this value.
@param		end		The first offset within character stream _NOT_
                                a part of this token.  Pass a negative value to
not set.
*/
void settoken(db_lexer_token_t *tokenp, db_uint8 type, db_int which,
              db_int start, db_int end) {
  tokenp->info = 0;
  tokenp->bcode = 0;
  tokenp->type = type;
...
db_int lexer_next(db_lexer_t *lexerp) {
  ...
  while (lexible(lexerp) && !done) {
    ...
    case DB_LEXER_MODE_NONE:
      ...
      newtoken.end++;
      if (tchar == '(') {
        settoken(&newtoken, DB_LEXER_TT_LPAREN, 0, -1, lexerp->offset);
        done = 1;
      } else if (tchar == ')') {
        settoken(&newtoken, DB_LEXER_TT_RPAREN, 0, -1, lexerp->offset);
        done = 1;
      } else if (tchar == ',') {
        settoken(&newtoken, DB_LEXER_TT_COMMA, 0, -1, lexerp->offset);
        done = 1;
      } else if (tchar == '?') {
        settoken(&newtoken, DB_LEXER_TT_PLACEHOLDER, 0, -1, lexerp->offset);
        if (1 == nextchar(lexerp, &tchar)) {
          if ('i' == tounicase(tchar)) {
            newtoken.info = DB_EETNODE_CONST_DBINT;
          } else if ('s' == tounicase(tchar)) {
            newtoken.info = DB_EETNODE_CONST_DBSTRING;
          } else {
            DB_ERROR_MESSAGE("incorrect placeholder type", lexerp->offset,
                             lexerp->command);
            return -1;
          }
        } else {
        ...
        if (!peekchar(lexerp, &tchar) ||
            (!isdigit_(tchar) && !isletter(tchar) && '*' != tchar &&
             tchar != DB_LEXER_QT_IDENT)) {
          DB_ERROR_MESSAGE("incorrect input after \".\"", lexerp->offset,
                           lexerp->command);
          return -1; // TODO: This is a hard problem.  Do we signal errors or
                     // not?  I'm not sure.
        } else if (isdigit_(tchar)) {
          mode = DB_LEXER_MODE_DECIMAL;
        } else {
          settoken(&newtoken, DB_LEXER_TT_IDENTCONJ, -1, lexerp->offset - 1,
                   lexerp->offset);
          done = 1;
        }
        ...
        settoken(&newtoken, DB_LEXER_TT_TERMINATOR, -1, lexerp->offset - 1,
                 lexerp->offset);
        done = 1;
      } else {
        // TODO: This still isn't right, reports offset of token after invalid
        // chracter.
    ...
    case DB_LEXER_MODE_STRING:
      if (!nextchar(lexerp, &tchar)) {
        DB_ERROR_MESSAGE("no matching end quote", lexerp->offset,
                         lexerp->command);
        return -1;
      } else if (tchar == DB_LEXER_QT_STRING) {
      ...
      if (!isoperator(&newtoken, lexerp, &which)) {
        ...
        if (token_stringequal(&newtoken, "--", 2, lexerp, 0)) {
          ...
          if (isoperator(&newtoken, lexerp, &which)) {
            ...
            DB_ERROR_MESSAGE("invalid operator", lexerp->offset,
                             lexerp->command);
            return -1;
          }
        }
...
db_int gettokenat(db_lexer_token_t *tokenp, db_lexer_t lexer, db_int offset,
                  db_uint8 skipnum) {
  lexer.offset = offset;
...
db_int whichclause(db_lexer_token_t *tokenp, db_lexer_t *lexerp) {
  return token_whichelement(tokenp, lexerp, clauses,
                            sizeof(clauses) / sizeof(struct keyword));
}
