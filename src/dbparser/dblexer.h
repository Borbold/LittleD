...
typedef enum db_lexer_token_info {
  ...
  DB_LEXER_TOKENINFO_TYPE_DBDECIMAL,     /**< @c DECIMAL keyword. */
...
typedef enum db_lexer_token_bcode {
  ...
  DB_LEXER_TOKENBCODE_CLAUSE_DELETE,         /**< @c DELETE command */
...
@param		lexerp		Pointer to the lexer instance @p tokenp belongs
                                to.
@returns	@c -1 if the token is not a clause, a non-negative integer
otherwise.
*/
