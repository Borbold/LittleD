...
db_int init_ntjoin(ntjoin_t *jp, db_eet_t *ep, db_op_base_t *lchild,
  ...
  jp->base.header->num_attr =
      lchild->header->num_attr + rchild->header->num_attr;
  jp->base.header->tuple_size =
      lchild->header->tuple_size + rchild->header->tuple_size;

  /* Allocate all necessary memory for the header. */
  jp->base.header->size_name =
      DB_QMM_BALLOC(mmp, sizeof(db_uint8) * (jp->base.header->num_attr));
  jp->base.header->names =
      DB_QMM_BALLOC(mmp, sizeof(char *) * (jp->base.header->num_attr));
  jp->base.header->types =
      DB_QMM_BALLOC(mmp, sizeof(db_uint8) * (jp->base.header->num_attr));
  jp->base.header->offsets =
      DB_QMM_BALLOC(mmp, sizeof(db_uint8) * (jp->base.header->num_attr));
  jp->base.header->sizes =
      DB_QMM_BALLOC(mmp, sizeof(db_uint8) * (jp->base.header->num_attr));
  ...
  db_uint8 first_roffset =
      lchild->header->offsets[i - 1] + lchild->header->sizes[i - 1];

  /* Write out right child's header properties new header.*/
  for (i = 0; i < (db_int)(rchild->header->num_attr); i++) {
    *(((jp->base.header->size_name) + i) + lchild->header->num_attr) =
        *((rchild->header->size_name) + i);
    *(((jp->base.header->names) + i) + lchild->header->num_attr) =
        *((rchild->header->names) + i);
    *(((jp->base.header->types) + i) + lchild->header->num_attr) =
        *((rchild->header->types) + i);
    *(((jp->base.header->offsets) + i) + lchild->header->num_attr) =
        first_roffset + *((rchild->header->offsets) + i);
    *(((jp->base.header->sizes) + i) + lchild->header->num_attr) =
        *((rchild->header->sizes) + i);
  }
  ...
  jp->lchild = lchild;
  jp->rchild = rchild;
  init_tuple(&(jp->lt), jp->lchild->header->tuple_size,
             jp->lchild->header->num_attr, mmp);
...
db_int next_ntjoin(ntjoin_t *jp, db_tuple_t *next_tp, db_query_mm_t *mmp) {
  ...
  db_tuple_t rt;
  init_tuple(&rt, jp->rchild->header->tuple_size, jp->rchild->header->num_attr,
             mmp);
    ...
    while (looper) {
      ...
      */
      ...
      if (1 == result && 1 == retval) {
         ...
         * left tuples bytes. */
        copytuplebytes(next_tp, &rt, jp->lchild->header->tuple_size, 0,
                       jp->rchild->header->tuple_size);
