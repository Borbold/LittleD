...
db_int setup_osijoin(osijoin_t *jp, db_query_mm_t *mmp) {
  if (NULL == jp->tree || NULL == jp->tree->nodes)
    return 0;
  ...
  if (rindexed < 0) {
    if (lindexed >= 0) {
      ...
      *((db_eetnode_attr_t *)(jp->uexpr->nodes)) = *rattrp;
      ...
      if (((((jp->rchild->header->num_attr) / 8) +
            ((jp->rchild->header->num_attr) % 8)) >
           (((jp->lchild->header->num_attr) / 8) +
            ((jp->lchild->header->num_attr) % 8))) ||
          ((jp->rchild->header->tuple_size) >
           (jp->lchild->header->tuple_size))) {
        close_tuple(&(jp->ut), mmp);
        init_tuple(&(jp->ut), jp->rchild->header->num_attr,
                   jp->rchild->header->tuple_size, mmp);
  ...
  db_uint8 lvalid = scan_find((scan_t *)indexed, jp->indexon, jp->uexpr,
                              &(jp->ut), unindexed->header, mmp);
...
db_int next_osijoin(osijoin_t *jp, db_tuple_t *next_tp, db_query_mm_t *mmp) {
  ...
  while (DB_OSIJOIN_UMORE(jp->bitinfo)) {
    if (DB_OSIJOIN_IVALID(jp->bitinfo) && 1 == next(indexed, it, mmp) &&
        1 == evaluate_eet(jp->tree, &result, tpa, hpa, 0, mmp) && 1 == result) {
    } else {
      DB_OSIJOIN_UPDATE_UMORE(jp->bitinfo, next(unindexed, &(jp->ut), mmp));
      lvalid = scan_find((scan_t *)indexed, jp->indexon, jp->uexpr, &(jp->ut),
                         unindexed->header, mmp);
      DB_OSIJOIN_UPDATE_IVALID(jp->bitinfo, lvalid);
      continue;
    }
