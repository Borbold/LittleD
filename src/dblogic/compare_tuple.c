...
db_int8 cmp_tuple(db_tuple_t *a, db_tuple_t *b, relation_header_t *hp_a,
  ...
  db_int type;
  for (i = 0; NULL != order_exprs_a && NULL != order_exprs_b && i < num_expr;
       i++) {
    type = evaluate_eet(&order_exprs_a[i], NULL, NULL, &hp_a, 0, mmp);
    if (order_exprs_a != order_exprs_b &&
        evaluate_eet(&order_exprs_b[i], NULL, NULL, &hp_b, 0, mmp) != type)
      exit(-1);
  ...
  if (cmp_result == 0 && hp_a == hp_b && 1 != strictComp) {
    ...
    for (i = 0; i < (db_int)hp_a->num_attr; ++i) {
      ...
      case DB_INT:
        cmp_result = getintbypos(a, i, hp_a) - getintbypos(b, i, hp_a);
        break;
      case DB_STRING:
        cmp_result =
            strcmp(getstringbypos(a, i, hp_a), getstringbypos(b, i, hp_a));
        break;
      case DB_DECIMAL:
        cmp_result = getdecimalbypos(a, i, hp_a) - getdecimalbypos(b, i, hp_a);
        break;
      default:
        break;
      }
...
db_int next_inorder_tuple(db_tuple_t *next_tp, db_tuple_t *previous_tp,
                          db_int *next_count, db_op_base_t *src_op,
                          db_eet_t *order_exprs, db_uint8 num_expr,
                          db_uint8 *order, db_query_mm_t *mmp) {
  ...
  if (1 == next(src_op, next_tp, mmp)) {
    ...
    if (previous_tp != NULL &&
      ...
      for (i = 0; i < src_op->header->tuple_size; ++i) {
        next_tp->bytes[(db_int)i] = previous_tp->bytes[(db_int)i];
      }
      for (i = 0; i < src_op->header->num_attr; ++i) {
        next_tp->isnull[((db_int)i) / 8] = previous_tp->isnull[((db_int)i) / 8];
      }
    }
  } else {
    return 0;
  ...
  while (1 == next(src_op, &t, mmp)) {
    // printf("First db_int of next: %d\n", getintbypos(next_tp, 0,
    // src_op->header)); printf("First db_int of t: %d\n", getintbypos(&t, 0,
    // src_op->header));
    /* If t comes after previous */
    if (previous_tp == NULL ||
        cmp_tuple(&t, previous_tp, src_op->header, src_op->header, order_exprs,
                  order_exprs, num_expr, order, 0, mmp) > 0) {
      ...
      case -1: /* t comes before next */
        ...
        db_uint8 i;
        for (i = 0; i < src_op->header->tuple_size; ++i) {
          next_tp->bytes[(db_int)i] = t.bytes[(db_int)i];
        }
        for (i = 0; i < src_op->header->num_attr; ++i) {
          next_tp->isnull[((db_int)i) / 8] = t.isnull[((db_int)i) / 8];
        }
        break;
