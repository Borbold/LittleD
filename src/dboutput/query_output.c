...
char *formatQuery(db_op_base_t *op, db_query_mm_t *mmp) {
  ...
  for (i = 0; i < (db_int)op->header->num_attr; ++i) {
    ...
    if (op->header->names == NULL || op->header->names[i] == NULL ||
        op->header->size_name == NULL || op->header->size_name[i] <= 0) {
      if ((db_uint8)DB_INT == op->header->types[i]) {
        sprintf(tmp, " %*s ", widths[i], "INTEGER");
        strcat(out, tmp);
      } else if ((db_uint8)DB_STRING == op->header->types[i]) {
        sprintf(tmp, " %*s(%d) ",
                widths[i] - numdbintdigits((db_int)op->header->sizes[i]) - 1,
                "STRING", (db_int)(op->header->sizes[i]) - 1);
        strcat(out, tmp);
      } else if ((db_uint8)DB_DECIMAL == op->header->types[i]) {
        sprintf(tmp, " %*s ", widths[i], "DECIMAL");
        strcat(out, tmp);
      }
    }
