...
/*TODO: DB_DECIMAL functionality is not implemented here on a par with DB_INT
 * and DB_STRING. If this code will be used, add DB_DECIMAL.*/
...
db_int init_aggregate(aggregate_t *ap, db_op_base_t *child, db_eet_t *exprs,
  ...
  for (i = 0; i < (db_int)num_expr; ++i) {
    if (num_aggr[i] == 0) {
      ...
      ap->aggr_locs[i][j] = -1;
      /* The end marker. */ // TODO: This is clearly broken for uint8.
    }
  }

  /* We will pre-compute types and jump_p's for all of the aggregate sub
   * expressions.
   */
  for (i = 0; i < (db_int)num_expr; ++i) {
    for (j = 0;; ++j) {
      if (ap->aggr_locs[i] != NULL && ap->aggr_locs[i][j] > -1) {
        db_eetnode_t *np =
            geteetnodebypos(&exprs[i], (db_int)(ap->aggr_locs[i][j]));
        if (np->type != (db_uint8)DB_EETNODE_AGGR_TEMP) {
          return -1;
        }
        np = ((db_eetnode_t *)(((db_eetnode_aggr_temp_t *)np) + 1));
        if (1 != subexpr_type_addr(
                     np,
                     &(((((db_eetnode_aggr_temp_t *)np) - 1))->subexpr_type),
  ...
  switch (
      createnewheader(ap->base.header, child->header, exprs, num_expr, mmp)) {
  case 0:
    return 0;
  case 1:
    break;
...
db_int aggr_prepare_expressions(aggregate_t *ap, db_eet_t *exprs,
                                db_int8 **aggr_locs, db_uint8 num_expr,
                                db_tuple_t *src_tp, db_tuple_t *tp,
                                db_query_mm_t *mmp) {
  db_int i, j;
  /* Evaluate non-aggregate values and setup temporary values for aggregate
   * functions. */
  for (i = 0; i < (db_int)num_expr; ++i) {
    for (j = 0;; ++j) {
      if (i < 0 || i > (db_int)num_expr ||
          (aggr_locs[i] != NULL && aggr_locs[i][j] < -1)) {
        return -1;
      }
      /* If there isn't an aggregate in this expression... */
      else if (NULL == aggr_locs[i]) {
        db_uint8 node_type = exprs[i].nodes->type;
        if ((node_type == DB_EETNODE_ATTR &&
             gettypebypos(ap->child->header,
                          ((db_eetnode_attr_t *)(exprs[i].nodes))->pos) ==
                 DB_INT) ||
            node_type == DB_EETNODE_CONST_DBINT ||
            (node_type >= DB_EETNODE_OP_NOT &&
             node_type <= DB_EETNODE_FUNC_LENGTH_DBSTRING)) {
          db_int result;
          switch (evaluate_eet(&(exprs[i]), &result, &src_tp,
                               &(ap->child->header), 0, mmp)) {
          case 1:
            /* Make sure this attributes isnull bit is 0. */
            tp->isnull[(i / 8)] &= ~(1 << (i % 8));
            *((db_int *)(&(tp->bytes[(db_int)(ap->base.header->offsets[i])]))) =
                result;
            break;
          case 2:
          ...
          default:
            return -1;
            // TODO: Might need to do more things here to prevent memory
            // leaks?!?
          }
        } else if ((node_type == DB_EETNODE_ATTR &&
                    gettypebypos(
                        ap->child->header,
                        ((db_eetnode_attr_t *)(exprs[i].nodes))->pos) ==
                        DB_STRING) ||
          ...
          char *result;
          db_int retval = evaluate_eet(&(exprs[i]), &result, &src_tp,
                                       &(ap->child->header), 0, mmp);
      ...
      /* If there is an aggregate in this expression... */
        ...
        if (DB_EETNODE_AGGR_TEMP !=
            geteetnodetypebypos(&(exprs[i]), (db_int)(aggr_locs[i][j]))) {
            ...
            for (l = 0;; ++l) {
              if (aggr_locs[k] != NULL) {
                if ((k < i || (k == i && l < j)) && aggr_locs[k][l] > -1) {
                  DB_QMM_BFREE(mmp,
                               ((db_eetnode_aggr_temp_t *)geteetnodebypos(
                                    &(exprs[k]), (db_int)(aggr_locs[k][l])))
                                   ->value_p);
                  ((db_eetnode_aggr_temp_t *)geteetnodebypos(
                       &(exprs[k]), (db_int)(aggr_locs[k][l])))
                      ->value_p = NULL;
        ...
        db_eetnode_aggr_temp_t *aggr_np =
            (db_eetnode_aggr_temp_t *)geteetnodebypos(&(exprs[i]),
                                                      (db_int)aggr_locs[i][j]);
            ...
            *((db_int *)(aggr_np->value_p)) = 0;
            ...
            if (aggr_np->aggr_type == DB_AGGR_MAX) {
              *((db_int *)(aggr_np->value_p)) = DB_INT_MIN;
                       ...
                       aggr_np->aggr_type == DB_AGGR_BAND) {
...
db_int next_aggregate(aggregate_t *ap, db_tuple_t *tp, db_query_mm_t *mmp) {
  ...
  */
  ...
  db_tuple_t src_t;
  init_tuple(&src_t, ap->child->header->tuple_size, ap->child->header->num_attr,
             mmp);
  ...
  */
  for (i = 0; i < ((db_int)(ap->base.header->num_attr)); ++i) {
    // TODO: This works, but its pretty darn ugly.  There is definitely a better
    // approach to this.
    tp->isnull[i / 8] |= (1 << (i % 8));
  }
  ...
  do {
    ...
    if (ap->previous_tp != NULL && looper == 0) {
      ...
      // TODO: This works, but its pretty darn ugly.  There is definitely a
    ...
    if (ap->previous_tp == NULL || looper == 0) {
      ...
      switch (next_inorder_tuple(&src_t, ap->previous_tp, &(ap->next_count),
                                 ap->child, (ap->groupby_exprs),
                                 ap->num_groupby_expr, orderings, mmp)) {
      case -1:
    ...
    /* Evaluate non-aggregate values and setup temporary values for aggregate
     * functions. */
    // TODO: Do something similar for HAVING clause only when
    // ap->having_aggr_locs == NULL.
    if (1 == aggr_prepare_expressions(ap, ap->exprs, ap->aggr_locs,
                                      ap->num_expr, &src_t, tp, mmp)) {
      /* Continue on. */
    } else {
      close_tuple(&src_t, mmp);
      return -1;
    }
    /* Abstraction point number 2: aggregation.  Only needs to be called on
     * HAVING clause if it has aggregates. */
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /* While the previous tuple shares the same groupby expression values
     * as returned tuple. */
    while (ap->next_count >= 0 &&
           (ap->previous_tp == NULL ||
            (0 == cmp_tuple(ap->previous_tp, &src_t, ap->child->header,
                            ap->child->header, ap->groupby_exprs,
                            ap->groupby_exprs, ap->num_groupby_expr, orderings,
                            1, mmp)))) {
      /* For each attribute... */
      for (i = 0; i < ap->num_expr; i++) {
        if (ap->aggr_locs[i] != NULL) {
          ...
          for (; ap->aggr_locs[i][j] > -1; ++j) {
            ...
            /* Get the type of the expression just after the aggregate, malloc
             * appropriately. */
            db_eetnode_aggr_temp_t *aggr_np =
                (db_eetnode_aggr_temp_t *)geteetnodebypos(
                    &(ap->exprs[i]), (db_int)(ap->aggr_locs[i][j]));
            if ((db_uint8)DB_EETNODE_CONST_DBINT == aggr_np->subexpr_type) {
              temp_p = DB_QMM_BALLOC(mmp, sizeof(db_int));
            } else if ((db_uint8)DB_EETNODE_CONST_DBSTRING ==
                       aggr_np->subexpr_type) {
            ...
            db_int retval = evaluate_eet(
                &(ap->exprs[i]), temp_p, &src_tp, &(ap->child->header),
                (db_uint8)(ap->aggr_locs[i][j] + 1), mmp);

            if (-1 == retval) {
              db_int k, l;
              for (k = 0; k < i; ++k) {
                for (l = 0;
                     ap->aggr_locs[k] != NULL && ap->aggr_locs[k][l] > -1;
                     ++l) {
                  DB_QMM_BFREE(
                      mmp, ((db_eetnode_aggr_temp_t *)geteetnodebypos(
                                &(ap->exprs[k]), (db_int)(ap->aggr_locs[k][l])))
                               ->value_p);
                  ((db_eetnode_aggr_temp_t *)geteetnodebypos(
                       &(ap->exprs[k]), (db_int)(ap->aggr_locs[k][l])))
                      ->value_p = NULL;
              ...
              if (aggr_np->aggr_type == DB_AGGR_COUNTROWS) {
                *((db_int *)(aggr_np->value_p)) =
                    *((db_int *)(aggr_np->value_p)) +
                    ((db_int)(ap->next_count));
              } else if ((db_uint8)DB_EETNODE_CONST_DBINT ==
                         aggr_np->subexpr_type) {
                if (aggr_np->aggr_type == DB_AGGR_BAND) {
                  ...
                  *((db_int *)(aggr_np->value_p)) =
                  ...
                  // *((db_int*)(aggr_np->value_p))); printf("Added temp value:
                  // %d\n", (*((db_int*)temp_p))); printf("Added temp count:
                  // %d\n", ap->next_count);
                  *((db_int *)(aggr_np->value_p)) =
                      *((db_int *)(aggr_np->value_p)) +
                      (((db_int)(ap->next_count)) * (*((db_int *)temp_p)));
      ...
      if (ap->previous_tp == NULL) {
        ...
        init_tuple(ap->previous_tp, ap->child->header->tuple_size,
      ...
      for (i = 0; i < (db_int)(ap->child->header->num_attr); ++i) {
        // FIXME: There has to be a better way to do this.  Involving %'s and
        // division.
        ap->previous_tp->isnull[i / 8] = src_t.isnull[i / 8];
      }

      /* FIXME: It would be better if we didn't have to create two variables
       * that are hopefully never used.... */
      /* Get the next tuple. */
      db_int k, l;
      switch ((next_inorder_tuple(&src_t, ap->previous_tp, &(ap->next_count),
                                  ap->child, (ap->groupby_exprs),
      ...
      default:
        for (k = 0; k < i; ++k) {
          for (l = 0; ap->aggr_locs[k] != NULL && ap->aggr_locs[k][l] > -1;
               ++l) {
            DB_QMM_BFREE(mmp,
                         ((db_eetnode_aggr_temp_t *)geteetnodebypos(
                              &(ap->exprs[k]), (db_int)(ap->aggr_locs[k][l])))
      ...
      // printf("First db_int in src_t: %d\n", getintbypos(&src_t, 0,
      // ap->child->header)); printf("First db_int in previous_tp: %d\n",
    ...
    /* Abstraction point number 3: do final expression evaluation. Call this on
       having if it has aggregate, otherwise just eval it (but do all of that
       where we currently eval HAVING clause. */
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /* For each expresion... */
    for (i = 0; i < ap->num_expr; ++i) {
      if (ap->aggr_locs[i] != NULL) {
        ...
        if ((db_uint8)(DB_EETNODE_AGGR_TEMP) != node_type) {
          if (node_type == DB_EETNODE_CONST_DBINT ||
              node_type == DB_EETNODE_CONST_DBSTRING ||
              node_type ==
                  DB_EETNODE_ATTR) /* TODO: Implement future types here. */
          {
            /* Error out.  We have constructed a clause that does not make
             * sense. */
            db_int k, l;
            for (k = 0; k < i; ++k) {
              for (l = 0; ap->aggr_locs[k] != NULL && ap->aggr_locs[k][l] > -1;
            ...
            db_tuple_t *src_tp = &src_t;
            retval = evaluate_eet(&(ap->exprs[i]), &result, &src_tp,
                                  &(ap->child->header), 0, mmp);
            if (-1 == retval) {
              db_int k, l;
              for (k = 0; k < i; ++k) {
        ...
        } else if (ap->aggr_locs[i] != NULL &&
                   (db_uint8)(DB_EETNODE_AGGR_TEMP) == node_type) {
          db_eetnode_aggr_temp_t *aggr_np = (db_eetnode_aggr_temp_t *)((
              db_eetnode_aggr_temp_t *)geteetnodebypos(&(ap->exprs[i]),
                                                       (db_int)0));
          if (aggr_np->aggr_type <= DB_AGGR_LAST) {
            db_int result, retval;
            db_tuple_t *src_tp = &src_t;
    ...
    db_int retval =
        evaluate_eet(ap->having_expr, &looper, &tp, &(ap->base.header), 0, mmp);
  ...
  for (k = 0; k < i; ++k) {
    for (l = 0; ap->aggr_locs[k] != NULL && ap->aggr_locs[k][l] > -1; ++l) {
      DB_QMM_BFREE(mmp, ((db_eetnode_aggr_temp_t *)geteetnodebypos(
                             &(ap->exprs[k]), (db_int)(ap->aggr_locs[k][l])))
                            ->value_p);
      ((db_eetnode_aggr_temp_t *)geteetnodebypos(&(ap->exprs[k]),
                                                 (db_int)(ap->aggr_locs[k][l])))
          ->value_p = NULL;
    }
  }
  close_tuple(&src_t, mmp);
