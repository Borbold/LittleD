...
db_int createTable(db_lexer_t *lexerp, db_int end, db_query_mm_t *mmp) {
  ...
  if (1 == lexer_next(lexerp) &&
    ...
    if (db_fileexists(tablename)) {
      DB_ERROR_MESSAGE("duplicate table name", lexerp->token.start,
                       lexerp->command);
  ...
  while (1 == lexer_next(lexerp) && DB_LEXER_TT_RPAREN != lexerp->token.type) {
    if (attrcount > 0 && DB_LEXER_TT_COMMA != lexerp->token.type) {
      DB_ERROR_MESSAGE("duplicate attr name", lexerp->token.start,
                       lexerp->command);
    ...
    if (DB_LEXER_TT_IDENT == lexerp->token.type) {
      ...
      if (1 == lexer_next(lexerp) &&
          DB_LEXER_TOKENINFO_TYPE_DBINT == lexerp->token.info) {
        arr[0].type = DB_INT;
      ...
      /* TODO: Implement future types here. */
        ...
        DB_ERROR_MESSAGE("bad attribute type", lexerp->token.start,
                         lexerp->command);
        db_qmm_bfree(mmp, arr);
        db_fileclose(newtable);
        db_fileremove(tablename);
      ...
      DB_ERROR_MESSAGE("bad attribute declaration", lexerp->token.start,
                       lexerp->command);
      db_qmm_bfree(mmp, arr);
      db_fileclose(newtable);
      db_fileremove(tablename);
  ...
  while (attrcount > 0) {
    ...
    db_uint8 length =
        (db_uint8)gettokenlength(&(arr[((int)attrcount) - 1])) + 1;
