...
db_int init_index(db_index_t *indexp, char *name) {
  ...
  if (DB_STORAGE_NOFILE == indexp->indexref ||
      1 != db_fileread(indexp->indexref, &(indexp->type), 1)) {
    return 0;
  }
...
db_int scan_find(scan_t *sp, db_uint8 indexon, db_eet_t *searchfor,
                 ...
                 db_query_mm_t *mmp) {
  db_index_offset_t offset = db_index_getoffset(
      sp, indexon, searchfor, comparator_tp, comparator_hp, mmp);
...
db_index_offset_t db_index_getoffset(scan_t *sp, db_uint8 indexon,
  ...
  if (DB_INDEX_TYPE_INLINE == index.type) {
    ...
    size_t total_size =
        sp->base.header->tuple_size + (sp->base.header->num_attr / 8);
    total_size += (sp->base.header->num_attr) % 8 > 0 ? 1 : 0;
    long last;
    if (sizeof(long) !=
        db_fileread(index.indexref, (unsigned char *)&(last), sizeof(long))) {
      return -1;
    }
    if (last > 0)
    ...
    db_tuple_t temp;
    init_tuple(&temp, sp->base.header->tuple_size, sp->base.header->num_attr,
               mmp);
    rewind_scan(sp, mmp);
    ...
    while (imin <= imax) {
      ...
      /* arr[imid], key */
      if (NULL ==
          comparator_hp) /* FIXME: quick hack to let indexed scans work. */
      {
        result = getintbypos(&temp, ((db_int)searchfor), sp->base.header) -
                 ((db_int)comparator_tp);
      } else
        result = cmp_tuple(&temp, comparator_tp, sp->base.header, comparator_hp,
                           sp->idx_meta_data.exprs[indexon], searchfor,
                           sp->idx_meta_data.num_expr[indexon], order, 1, mmp);
    ...
    /* FIXME: quick hack to let indexed scans work. (first part of the
     * condition) */
    if (NULL != comparator_hp &&
        0 != cmp_tuple(&temp, comparator_tp, sp->base.header, comparator_hp,
                       sp->idx_meta_data.exprs[indexon], searchfor,
                       sp->idx_meta_data.num_expr[indexon], order, 1, mmp))
      i = -1;
