...
typedef enum {
  DB_SCAN =
      0, /**< File based scan operator. @todo Write table-based equivalent. */
...
typedef struct {
  ...
  db_int end;         /**< First character following tables name. */
  db_int fname_start; /**< First character in relation name.*/
  db_int fname_end;   /**< First character following relation name. */
  long tuple_start;   /**< Offset of first tuple record. */
  db_index_meta_t idx_meta_data; /**< File containing metadata for index. */
...
@todo		Maybe change lt to be a pointer such that it could be
interchanged with indexon? would have to deal with number issue, unless pointer
                was to a struct with pointer and size... maybe.  Same goes for
lvalid/bitinfo.
