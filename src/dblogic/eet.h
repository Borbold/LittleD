...
typedef enum {
  ...
  DB_EETNODE_CONST_DBDECIMAL, /**< An decimal constant. */
...
/* Extended constant decimal node type. */
/**
@struct		db_eetnode_dbdecimal_t
@brief		The constant decimal EET node type.
*/
typedef struct {
  /*@{*/
  db_eetnode_t base;  /**< The supertype of this node. */
  db_decimal decimal; /**< The value of the constant. */
                      /*@}*/
} db_eetnode_dbdecimal_t;
...
*/
db_int createnewheader(relation_header_t *hp, relation_header_t *old_hp,
...
@returns	@c 1.	@todo Figure out if there is any errors that we could
detect here.
*/
...
@param		pos		The position of the node we wish to get the type
of.
@returns	@c 1.	@todo Figure out if there is any errors that we could
detect here.
@todo		Use this method in all other "advance pointers n times" type of
...
*/
db_int evaluate_eet(db_eet_t *eet, void *rp, db_tuple_t **tp,
